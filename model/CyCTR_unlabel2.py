...
from model.cyc_transformer4_unlabel2 import CyCTransformer
...
class CyCTR(nn.Module):
    ...
    def forward(self, x, s_x=torch.FloatTensor(1,1,3,473,473).cuda(), s_y=torch.FloatTensor(1,1,473,473).cuda(), y=None, padding_mask=None, s_padding_mask=None,u_padding=None,un = None,un_strong = None):
        ...
        if self.training:
            ...
            unstrong_fea = []
            for i in range(self.num_unlabel):
                unlabel_cat_feat = [unlabel_feat[:,i,...], global_supp_pp.expand(-1, -1, fts_size[0], fts_size[1]), corr_unlabel_mask[:,i,...]]
                unlabel_fea.append(self.qry_merge_feat(torch.cat(unlabel_cat_feat, dim=1)))
                unstrong_cat_feat = [unstrong_feat[:,i,...], global_supp_pp.expand(-1, -1, fts_size[0], fts_size[1]), corr_unstrong_mask[:,i,...]]
        ...
        if self.with_transformer:
            ...
            query_feat_list = self.transformer(query_feat, padding_mask.float(), aug_supp_feat, s_y.clone().float(),
                                               s_padding_mask.float(), unlabel_feat)
        ...
        if self.training:
            ...
            for i in range(self.num_unlabel):
                ...
                # print('fused_unlabel', fused_unlabel)
                for i in range(self.num_unlabel):
                    out_unlabel = self.cls(fused_unlabel[i])  # [2, 2, 60, 60]
                    out_unstrong = self.cls(fused_unstrong[i])  # [2, 2, 60, 60]
                    # Compute unsupervised loss
