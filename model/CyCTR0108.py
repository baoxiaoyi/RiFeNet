...
class CyCTR(nn.Module):
    def __init__(self, layers=50, classes=2, shot=1, reduce_dim=384, \
        ...
        self.qry_merge_feat = nn.Sequential(
                    #nn.Conv2d(reduce_dim*18+prior_channel, reduce_dim, kernel_size=1, padding=0, bias=False),
    ...
    def forward(self, x, s_x=torch.FloatTensor(1,1,3,473,473).cuda(), 
        ...
        #tmp_query = torch.cat([qry_bcb_fts['2'], qry_bcb_fts['3']], dim=1)
        #query_feat = self.adjust_feature_qry(query_feat)
        ...
        if self.training:
            ...
            for i in range(self.num_unlabel):
                ...
                unlabel_cat_feat = [unlabel_feat[:,i,...], global_supp_pp.expand(-1, -1, fts_size[0], fts_size[1]), global_query_pp.expand(-1, -1, fts_size[0], fts_size[1]),corr_unlabel_mask[:,i,...]]
                unlabel_fea.append(self.qry_merge_feat(torch.cat(unlabel_cat_feat, dim=1)))
                unstrong_cat_feat = [unstrong_feat[:,i,...], global_supp_pp.expand(-1, -1, fts_size[0], fts_size[1]), global_query_pp.expand(-1, -1, fts_size[0], fts_size[1]), corr_unstrong_mask[:,i,...]]
        ...
        if self.with_transformer:
            ...
            if self.training:
                fused_unlabel = []
                fused_unstrong = []
                for j in range(self.num_unlabel):
                    unlabel_featt=self.merge_res(unlabel_feat[:,j,...]) + unlabel_feat[:,j,...]
                    unstrong_featt=self.merge_res(unstrong_feat[:,j,...])+unstrong_feat[:,j,...]
                    fused_unlabel.append(unlabel_featt)
                    fused_unstrong.append(unstrong_featt)
