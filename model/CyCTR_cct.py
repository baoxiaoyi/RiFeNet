...
from model.loss import WeightedDiceLoss,WeightedDiceLoss2
from model.cyc_transformer2_cct import CyCTransformer
...
class CyCTR(nn.Module):
    def __init__(self, layers=50, classes=2, shot=1, reduce_dim=384, \
        ...
        upscale = 473/60     # v3+ only need 4
        num_out_ch = 256
        decoder_in_ch = num_out_ch
        drop_decoder = [DropOutDecoder(upscale, decoder_in_ch, classes,
                                       drop_rate=conf['drop_rate'], spatial_dropout=conf['spatial'])
                        for _ in range(conf['drop'])]
        cut_decoder = [CutOutDecoder(upscale, decoder_in_ch, classes, erase=conf['erase'])
                       for _ in range(conf['cutout'])]
        context_m_decoder = [ContextMaskingDecoder(upscale, decoder_in_ch, classes)
                             for _ in range(conf['context_masking'])]
        object_masking = [ObjectMaskingDecoder(upscale, decoder_in_ch, classes)
                          for _ in range(conf['object_masking'])]
        feature_drop = [FeatureDropDecoder(upscale, decoder_in_ch, classes)
                        for _ in range(conf['feature_drop'])]
        feature_noise = [FeatureNoiseDecoder(upscale, decoder_in_ch, classes,
    ...
    def forward(self, x, s_x=torch.FloatTensor(1,1,3,473,473).cuda(), s_y=torch.FloatTensor(1,1,473,473).cuda(), y=None, padding_mask=None, s_padding_mask=None,un = None):
        ...
        unlabel_feat_high = unlabel_bcb_fts['3']
        ...
        corr_unlabel_mask = self.generate_prior(unlabel_feat_high, supp_feat_high, s_y, fts_size)
        ...
        unlabel_cat_feat = [unlabel_feat, global_supp_pp.expand(-1, -1, fts_size[0], fts_size[1]), corr_unlabel_mask]
        unlabel_feat = self.qry_merge_feat(torch.cat(unlabel_cat_feat, dim=1))
        ...
        if self.with_transformer:
            ...
            unlabel_feat_list = self.transformer(unlabel_feat, torch.zeros_like(padding_mask).float(), aug_supp_feat, s_y.clone().float(),s_padding_mask.float())
            fused_query_feat = []
            ...
            fused_unlabel_feat = self.merge_multi_lvl_sum(fused_unlabel_feat)+fused_unlabel_feat
        ...
        ''' Below is semi loss '''
        out_unlabel = self.cls(fused_unlabel_feat)
        #print('out',out_unlabel.size())[2, 2, 60, 60]
        output_unsup_main = F.interpolate(out_unlabel, size=(h, w), mode='bilinear', align_corners=True)
        #print('output_unsup_main', output_unsup_main.size())[2, 2, 473, 473]


        # output of aux decoders
        outputs_unsup = [aux_decoder(fused_unlabel_feat, output_unsup_main.detach()) for aux_decoder in self.aux_decoders]
        targets = F.softmax(output_unsup_main.detach(), dim=1)
        #outputs_unsup = F.interpolate(outputs_unsup, size=(h, w), mode='bilinear', align_corners=True)
        #print('unsup',outputs_unsup[0].size())[2, 2, 473, 473]
        # Compute unsupervised loss
        #loss_unsup = sum([self.criterion2(u, targets) for u in outputs_unsup])
