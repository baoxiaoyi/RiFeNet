...
def attention(query, key, value, mask=None, dropout=None):
    ...
    #WHEN Q K V SIZE:
    #q2 torch.Size([1, 8, 1190, 32]) torch.Size([1, 8, 289, 32]) torch.Size([1, 8, 289, 32])
    d_k = query.size(-1)
    scores = torch.matmul(query, key.transpose(-2, -1)) \
             / math.sqrt(d_k)
    if mask is not None:
        scores = scores.masked_fill(mask == 0, -1e9)
    p_attn = F.softmax(scores, dim=-1) #torch.Size([1, 8, 268, 466])
    if dropout is not None:
        p_attn = dropout(p_attn)
    #print('p_attn',p_attn.size())#[1, 8, 1190, 289])
    #print(value.size())#[1, 8, 289, 32]
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        #input qkv size
        #q1 torch.Size([1, 268, 256])
            #torch.Size([1, 466, 256])
            #torch.Size([1, 268, 256])
            #print('q1',query.size())
            #print(key.size())
            #(value.size())
        ...
        #q2 torch.Size([1, 8, 268, 32])
        #torch.Size([1, 8, 466, 32])
        #torch.Size([1, 8, 268, 32])
...
class CyCTransformer(nn.Module):
    def __init__(self,
        ...
        for l_id in range(self.num_layers):
            if self.use_unlabel:
                self.unlabel_layers.append(
                    MultiHeadedAttention(h=12 if embed_dims%12==0 else self.num_heads, d_model=embed_dims, dropout=self.dropout),
                )
        ...
        if self.use_unlabel:
            self.unlabel_layers  = nn.ModuleList(self.unlabel_layers)
        if self.use_cross:
    ...
    def get_unlabel_flatten_input(self, x):
        #print('x:',x.size()) [10, 256, 60, 60])
        u_x_flatten = []
        u_x = x.view(-1, self.num_unlabel, x.size(1), x.size(2), x.size(3))
        #print('u_x:', u_x.size()) [1, 10, 256, 60, 60]
        for st_id in range(u_x.size(1)):
            u_x_s = u_x[:, st_id, ...]
            u_x_s = u_x_s.flatten(2).permute(0, 2, 1)  # [bs, c, h*w] -> [bs, h*w, c]
            u_x_flatten.append(u_x_s)

        pos_embedtotal = []
        u_x_flatten = torch.cat(u_x_flatten, 1) # [bs, h*w*shot, c] 1x[36000,256]
        bs,_,c, h, w = u_x.shape
        padding = torch.zeros(size=(bs, h, w),dtype=int,device=u_x_flatten.device)
        for st_id in range(self.num_unlabel):
            pos_embed = self.positional_encoding(padding)
            pos_embed = pos_embed.flatten(2).transpose(1, 2)
            pos_embedtotal.append(pos_embed)
        pos_embedtotal = torch.cat(pos_embedtotal,1)
        #u_x_flatten = torch.as_tensor(u_x_flatten,device=pos_embedtotal.device)
        return u_x_flatten, pos_embedtotal

    def sparse_unlabel_sampling(self, u_x):#[1.36000,256]
        if self.training:
            scale_min = 0.6
        ...
        re_arrange_k = []
        for b_id in range(u_x.size(0)):
            k_b = u_x[b_id]  # [num_elem, c]
            num_fg = u_x.size()[1]
            fg_select_idx = torch.randperm(num_fg)[:rand_fg_num]
            re_k = k_b[fg_select_idx]
            re_arrange_k.append(re_k)
        k = torch.stack(re_arrange_k, dim=0)#[1, 256, 256]
        return k
    ...
    def get_supp_flatten_input(self, s_x, supp_mask, s_padding_mask):
        ...
        #print('s_x',s_x.size())[1, 1, 256, 60, 60]
    ...
    def sparse_sampling(self, s_x, supp_mask, supp_valid_mask):
        ...
        k = torch.stack(re_arrange_k, dim=0)#[1, 1190每次不同, 256]
    ...
    def forward(self, x, qry_masks, s_x, supp_mask, s_padding_mask, u_x):
        ...
        u_x ,un_pos_embed_flatten = self.get_unlabel_flatten_input(u_x)#bs h*w*n_unlabel, c
        u_x = u_x + un_pos_embed_flatten#[1, 36000, 256])
        ...
        for l_id in range(self.num_layers):
            if self.use_self:
                #uk = self.sparse_unlabel_sampling(u_x)
                uk = self.unlabel_layers[l_id](q,u_x, u_x)
                #print('uk',uk.size())
                #print(q.size())
                q = q + uk
