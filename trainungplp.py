...
from model.CyCTRablaungplp import CyCTR
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    train_data = dataset.SemData(split=args.split, shot=args.shot, data_root=args.data_root, \
                                data_list=args.train_list, transform=train_transform,
                                 strong_transform = True,
                                 mode='train', \
                                use_coco=args.use_coco, use_split_coco=args.use_split_coco,num_unlabel=args.num_unlabel)
...
def train(train_loader, model, optimizer, transformer_optimizer, epoch, base_lrs):
    ...
    for i, (input, target, s_input, s_mask, padding_mask, s_padding_mask, u_padding, unlabeled_x, un_strong,subcls) in enumerate(train_loader):
        ...
        un_strong = un_strong.cuda(non_blocking=True)
        output, main_loss, aux_loss,loss_unsup = model(s_x=s_input, s_y=s_mask, x=input, y=target, padding_mask=padding_mask, s_padding_mask=s_padding_mask,u_padding = u_padding, un = unlabeled_x,un_strong = un_strong)
        if not args.multiprocessing_distributed:
            main_loss, aux_loss, loss_unsup = torch.mean(main_loss), torch.mean(aux_loss), torch.mean(loss_unsup)
        loss = main_loss + args.aux_weight * aux_loss + args.un_weight * loss_unsup
        '''for name, parms in model.named_parameters():
        a = None if parms.grad==None else torch.mean(parms.grad)
        if a:
            print('-->name:', name, '--weight', torch.mean(parms.data),' -->grad_value:', a)'''
        #print(loss_unsup)
        optimizer.zero_grad()
        ...
        n = input.size(0)
        if args.multiprocessing_distributed:
            main_loss, aux_loss, loss_unsup, loss = main_loss.detach() * n, aux_loss * n, loss_unsup * n, loss * n
            count = target.new_tensor([n], dtype=torch.long)
            dist.all_reduce(main_loss), dist.all_reduce(aux_loss), dist.all_reduce(loss_unsup), dist.all_reduce(loss), dist.all_reduce(count)
            n = count.item()
            main_loss, aux_loss, loss_unsup, loss = main_loss / n, aux_loss / n, loss_unsup/n, loss / n
        ...
        loss_unsup_meter.update(loss_unsup.item(), n)
        ...
        if (i + 1) % args.print_freq == 0 and main_process():
            logger.info('Epoch: [{}/{}][{}/{}] '
                        ...
                        'loss_unsup_meter {loss_unsup_meter.val:.4f}'
                        ...
                        'Accuracy {accuracy:.4f}.'.format(epoch+1, args.epochs, i + 1, len(train_loader),
                                                          ...
                                                          loss_unsup_meter=loss_unsup_meter,
...
def validate(val_loader, model, criterion=nn.CrossEntropyLoss(ignore_index=255)):
    ...
    for e in range(10):
        for i, (input, target, s_input, s_mask, padding_mask, s_padding_mask, u_padding, unlabeled_x,subcls, ori_label) in enumerate(val_loader):
            ...
            output = model(s_x=s_input, s_y=s_mask, x=input, y=target, padding_mask=padding_mask, s_padding_mask=s_padding_mask,u_padding = u_padding, un = unlabeled_x)
